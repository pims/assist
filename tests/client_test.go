...
func configureClient(t *testing.T) (*httptest.Server, *assist.Client) {

	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		fixturePath := urlToFixturePath(r.URL.String())
	...
	config := assist.NewConfig("XXXX", ts.URL)
	return ts, assist.NewClient(config)
}

func TestTeamsShots(t *testing.T) {
	ts, client := configureClient(t)
	defer ts.Close()
