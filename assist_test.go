...
func TestUserSerDer(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Failed unmarshalling user: %s", err)
	}
	...
	if user.LikesCount != 34954 {
		t.Errorf("Got = %v; want %v", user.LikesCount, 34954)
	}

	if len(user.Links) != 2 {
		t.Errorf("Got = %v; want %v", len(user.Links), 2)
	}
}

func TestShotWithTeamSerDer(t *testing.T) {
	...
	shot := &Shot{}
	err := json.Unmarshal([]byte(jsonString), shot)
	if err != nil {
		t.Errorf("Failed unmarshalling user: %s", err)
	}
	...
	gotLenTag := len(shot.Tags)
	wantLenTag := 4

	if gotLenTag != wantLenTag {
		t.Errorf("Got = %v; want %v", gotLenTag, wantLenTag)
	}

	gotTeamId := shot.Team.Id
	wantTeamId := 39

	if gotTeamId != wantTeamId {
		t.Errorf("Got = %v; want %v", gotTeamId, wantTeamId)
	}
}

func TestShotNoTeamSerDer(t *testing.T) {
	...
	if gotTeam != nil {
		t.Errorf("Got = %v; want %v", gotTeam, nil)
	}
}

func TestTeamSerDer(t *testing.T) {
	jsonString := `
[
  {
    "id" : 471756,
	...
	shots := make([]*Shot, 0)
	err := json.Unmarshal([]byte(jsonString), &shots)
	if err != nil {
		t.Errorf("Failed unmarshalling user: %s", err)
	}

	if len(shots) != 1 {
		t.Errorf("Length doesn't match. Expected: %d. Got: %d", 1, len(shots))
	}
}
